...
int solve(int left, int right){
    ...
    for(int i = left; i <= right; i++){
        if(pow(points[i].first - midX, 2) < minDist) // x좌표가 midX에서 minDist보다 작은 점들만 후보에 넣음
            candidate.push_back(points[i]);
    }

    sort(candidate.begin(), candidate.end(), cmp); // y좌표 기준으로 정렬

    for(int i = 0; i < candidate.size(); i++){
        for(int j = i+1; j < candidate.size(); j++){
            if(pow(candidate[i].second - candidate[j].second, 2) > minDist) // y좌표 차이가 minDist보다 크면 중단
                break;
