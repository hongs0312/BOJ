...
pV vec_process(pV a, pV b, ll k); //get a+kb
...
int main() {
    ...
    vector<pV> p(n), v(n); //p == place of star, v == velocity of star
    for(int i = 0; i < n; i++)
        cin >> p[i].first >> p[i].second >> v[i].first >> v[i].second;

    ll l, r, lo = 0, hi = t, fl, fr;
    vector<pV> lp(n), rp(n); //star's places when t == p and t == q
    while(hi-lo > 2) {
        l = (lo*2 + hi)/3;
        r = (lo + hi*2)/3;

        //get star's place when each time
        for(int i = 0; i < n; i++) {
            lp[i] = vec_process(p[i], v[i], l);
            rp[i] = vec_process(p[i], v[i], r);
        }
        fl = rotating_callipus(get_convex(lp));
        fr = rotating_callipus(get_convex(rp));
    ...
    vector<pV> tmp(n);
    int min_dist_day = 0;
    ll dist, min_dist = rotating_callipus(get_convex(p));
    for(int i = lo; i <= hi; i++) {
        for(int j = 0; j < n; j++) {
            tmp[j] = vec_process(p[j], v[j], i);
        }
        dist = rotating_callipus(get_convex(tmp));
        if(dist < min_dist) {
            min_dist = dist;
            min_dist_day = i;
        }
